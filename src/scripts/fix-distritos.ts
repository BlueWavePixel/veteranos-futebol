...
// Mapeamento completo dos 308 concelhos de Portugal → Distritos
const CONCELHO_DISTRITO: Record<string, string> = {
  // Aveiro
  ...
  "Penafiel": "Porto", "Porto": "Porto",
  "Póvoa de Varzim": "Porto", "Santo Tirso": "Porto",
  "Trofa": "Porto", "Valongo": "Porto",
  ...
  "Santarém": "Santarém", "Sardoal": "Santarém",
  ...
  "Penedono": "Viseu", "Resende": "Viseu",
  "Santa Comba Dão": "Viseu", "São João da Pesqueira": "Viseu",
  "São Pedro do Sul": "Viseu", "Sátão": "Viseu",
  "Sernancelhe": "Viseu", "Tabuaço": "Viseu", "Tarouca": "Viseu",
  "Tondela": "Viseu", "Vila Nova de Paiva": "Viseu", "Viseu": "Viseu",
  "Vouzela": "Viseu",
  // Regiões autónomas
  "Funchal": "Madeira", "Câmara de Lobos": "Madeira", "Machico": "Madeira",
  "Ribeira Brava": "Madeira", "Santa Cruz": "Madeira", "Santana": "Madeira",
  "São Vicente": "Madeira", "Porto Santo": "Madeira",
  "Calheta": "Madeira", "Ponta do Sol": "Madeira", "Porto Moniz": "Madeira",
  "Angra do Heroísmo": "Açores", "Calheta (Açores)": "Açores",
  "Corvo": "Açores", "Horta": "Açores", "Lagoa (Açores)": "Açores",
  "Lajes das Flores": "Açores", "Lajes do Pico": "Açores",
  "Madalena": "Açores", "Nordeste": "Açores",
  "Ponta Delgada": "Açores", "Povoação": "Açores",
  "Praia da Vitória": "Açores", "Ribeira Grande": "Açores",
  "Santa Cruz da Graciosa": "Açores", "Santa Cruz das Flores": "Açores",
  "São Roque do Pico": "Açores", "Velas": "Açores",
  "Vila do Porto": "Açores", "Vila Franca do Campo": "Açores",
};

async function main() {
  const db = drizzle(neon(process.env.DATABASE_URL!));

  // Buscar todas as equipas ativas com concelho
  const allTeams = await db.execute(sql`
    SELECT id, name, concelho, distrito, location
    FROM teams
    WHERE is_active = true AND concelho IS NOT NULL
    ORDER BY name
  `);
  ...
  for (const team of allTeams.rows as any[]) {
    const concelho = team.concelho.trim();
    const correctDistrito = CONCELHO_DISTRITO[concelho];

    if (!correctDistrito) {
      if (!missing.includes(concelho)) missing.push(concelho);
      notFound++;
      continue;
    }

    if (team.distrito !== correctDistrito) {
      const newLocation = `${concelho} / ${correctDistrito}`;
      await db.execute(sql`
        UPDATE teams
        SET distrito = ${correctDistrito},
            location = ${newLocation},
            updated_at = NOW()
        WHERE id = ${team.id}::uuid
      `);
      if (team.distrito !== correctDistrito) {
        console.log(`✓ ${team.name}: ${team.distrito || "sem"} → ${correctDistrito}`);
        updated++;
      }
    }
  }
  ...
  console.log(`Concelho não encontrado no mapa: ${notFound}`);
  if (missing.length > 0) {
    console.log(`\nConcelhos sem mapeamento:`);
    for (const c of missing.sort()) {
      console.log(`  - "${c}"`);
    }
  }
}
